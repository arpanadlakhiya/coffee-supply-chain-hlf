...
func (tc *RetailerContract) GrowBatch(ctx contractapi.TransactionContextInterface, batchID string) (bool, error) {
	txnID := ctx.GetStub().GetTxID()

	utils.LogMessage("RetailerContract.GrowBatch", "Recieved transaction to store a grown batch", batchID, txnID)

	if batchID == "" {
		return false, fmt.Errorf("RetailerContract.GrowBatch: Batch ID is empty")
	}
	...
	utils.LogMessage("RetailerContract.GrowBatch", "Stored batch data on ledger", batchID, ctx.GetStub().GetTxID())
